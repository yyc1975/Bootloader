 ...
 * @param[out]  u8 *z: the shared secret Z
 * @param[in]  u8 *d: the secret key for the entity U
 * @param[in] u8 *xV: the V ephemeral public key X coordinate
 * @param[in] u8 *yV: the V ephemeral public key Y coordinate
 * @param[in] u32 configuration: the configuration variable for transmitting the curve identifier
 * @return Error code
 * 
 ...
 * @param[out]  u8 *derivingkeymaterial: the derived key
 * @param[in]  u32 keydatalen: the bits length of the derived key material
 * @param[in] u8 *z: the shared secret
 * @param[in] u32 zbytelength: the shared secret length in bytes
 * @param[in] u8 *otherinfo: the OtherInfo field as defined in section 5.8.1.2
 * @param[in] u32 otherinfolength: the OtherInfo field bit length; this number is a multiple of 8
 * @param[in] u32 configuration: the configuration variable for transmitting the hash function identifier
 * @return Error code
 * 
 * @retval #UCL_OK              No error occurred
 * @retval #UCL_INVALID_INPUT  if one of the inputs is the pointer NULL
 ...
 * @ingroup UCL_ECC_SP80056 */
int ucl_sp800_56_concat_kdf(u8 *derivedkeyingmaterial, u32 keydatalen, u8 *z, u32 zbytelength, u8 *otherinfo, u32 otherinfobitlength, u32 configuration);
 ...
 *
 * @param[out]  u8 *z: the shared secret Z
 * @param[out]  u8 *d: the newly generated secret key for the entity U
 * @param[out] u8 *xU: the newly generated U ephemeral key X coordinate using the ucl_type_ecc_u8_affine_point structure
 * @param[out] u8 *yU: the newly generated U ephemeral key Y coordinate using the ucl_type_ecc_u8_affine_point structure
 * @param[in] u8 *xV: the V ephemeral public key X coordinate using the ucl_type_ecc_u8_affine_point structure
 * @param[in] u8 *yV: the V ephemeral public key Y coordinate using the ucl_type_ecc_u8_affine_point structure
 * @param[in] u8 *otherinfo: the OtherInfo field as defined in section 5.8.1.2
 * @param[in] u32 otherinfolength: the OtherInfo field bit length; this number is a multiple of 8
 * @param[in] u32 configuration: the configuration variable for transmitting the curve and hash function identifiers
 * @return Error code
 * 
 * @retval #UCL_OK              No error occurred
 * @retval #UCL_INVALID_INPUT  if one of the inputs is the pointer NULL
 ...
 * @limitation: the OtherInfo length is expressed in bits but is a multiple of 8 (so bytes).
 * @ingroup UCL_ECC_SP80056 */
int ucl_sp800_56_eum_ecc_cdh(u8 *derivedkeyingmaterial, u32 keydatalen, u8 *dU, u8 *xU, u8 *yU, u8 *xV, u8 *yV, u8 *otherinfo, u32 otherinfobitlength, u32 configuration);
 ...
 *
 * @ingroup UCL_ECC_SP80056 */
int ucl_ecc_sp800_56_ecc_cdh(u8 *z, u8 *dU,ucl_type_ecc_u8_affine_point V, ucl_type_curve *curve_params);
 ...
 * @limitation: the OtherInfo length is expressed in bits but is a multiple of 8 (so bytes).
 * @ingroup UCL_ECC_SP80056 */
int ucl_ecc_sp800_56_eum_ecc_cdh(u8 *derivedkeyingmaterial, u32 keydatalen, u8 *dU, ucl_type_ecc_u8_affine_point U,ucl_type_ecc_u8_affine_point V, u8 *otherinfo, u32 otherinfobitlength, ucl_type_curve *curve_params,u32 configuration);
